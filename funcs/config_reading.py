...
def read_datasets_configs():
  ...
  for version_key in ["small", "normal"]:
    ...
    if not isinstance(version["flanks"], int):
      raise TypeError(f"Incorrect type for 'flanks' in 'version[{version_key}]': Expected int, but got {type(version['flanks']).__name__}")
