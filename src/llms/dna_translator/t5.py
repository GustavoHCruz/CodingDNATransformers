...
class DNATranslatorT5(BaseModel):
	...
	def _prepare_dna(
		...
		return f"".join(f"[DNA_{nucl.upper()}]" for nucl in dna)
	
	def _prepare_protein(
		self,
		protein: str
	) -> str:
		return f"".join(f"[PROT_{amino_acid}]" for amino_acid in protein)
	...
	def _build_input(
		...
		input_sequence = f"<|DNA|>{self._prepare_dna(data['dna'])}"
		...
		if protein:
			output_sequence = f"<|PROTEIN|>{self._prepare_protein(protein)}"
