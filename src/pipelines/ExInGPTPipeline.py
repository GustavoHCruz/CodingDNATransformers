...
class ExInGPTPipeline(Pipeline):
	def _build_prompt(
		self,
		sequence: str,
		...
		out = f"<|SEQUENCE|>{process_sequence(sequence[:512])}\n"
	...
	def preprocess(
		...
		**preprocess_parameters
		...
		max_length = preprocess_parameters.get("max_length", 512)
	...
	def _forward(self, input_tensors: dict, **forward_params):
		...
		generate_kwargs = dict(forward_params)

		generate_kwargs.setdefault("max_new_tokens", 1)
		generate_kwargs.setdefault("pad_token_id", getattr(self.tokenizer, "eos_token_id", None))
		...
		self.model.eval()
		with torch.no_grad():
			output_ids = self.model.generate(**expanded_inputs, **generate_kwargs)

		return ModelOutput({"sequences": output_ids})
	...
	def postprocess(self, model_outputs: dict, **kwargs):
		...
		if output_ids.dim() == 1:
			last_id_tensor = output_ids[-1].unsqueeze(0)
		else:
			last_id_tensor = output_ids[0, -1].unsqueeze(0)

		raw_label = self.tokenizer.decode(last_id_tensor)
