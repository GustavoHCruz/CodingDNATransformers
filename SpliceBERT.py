	...
	def __getitem__(self, idx):
		...
		if len(self.data["organism"]) > idx and self.data["organism"][idx]:
			if random.random() > self.feat_hide_prob:
				prompt += f"Organism:{self.data["organism"][idx][:10]}[SEP]"
		
		if len(self.data["gene"]) > idx and self.data["gene"][idx]:
			if random.random() > self.feat_hide_prob:
				prompt += f"Gene:{self.data["gene"][idx][:10]}[SEP]"
...
class SpliceDNABERT():
	...
	def predict_single(self, data, map_pred=True):		
		...
		prompt = f"Sequence:{data["sequence"]}[SEP]"
		for key in keys:
			if hasattr(data, key):
				prompt += f"{key.capitalize()}:{data[key]}[SEP]"
		prompt += "Answer:"
...
splicebert.train(save_at_end="models/splicebert", lr=5e-5)
...
class SpliceBERT():
	...
	def train(self, lr=5e-5, epochs=3, save_at_end=None, evaluation=True):
		if not hasattr(self, "train_dataloader"):
			raise ValueError("Can't find the train dataloader, make sure you initialized it.")
