...
class SpliceDNABERT():
	...
	def __init__(self, checkpoint="zhihan1996/DNA_bert_6", device="cuda", seed=None, notification=False):
		"""
	...
	def add_train_data(self, data, sequence_len=512, batch_size=32, train_percentage=0.8):
		...
		dataset = SpliceBERTDataset(data, self.tokenizer, max_length=sequence_len)
	...
	def add_test_data(self, data, sequence_len=512, batch_size=32):
		if hasattr(self, "_last_train_info"):
			...
			self._last_train_info["batch_size"] != batch_size:
		...
		self.test_dataset = SpliceBERTDataset(data, self.tokenizer, sequence_len=sequence_len)
	...
	def train(self, lr=1e-9, epochs=15, save_at_end=None, evaluation=True):
	...
	def predict_single(self, sequence, map_pred=True):
		prompt = self._process_sequence(sequence=sequence)
...
splicebert = SpliceDNABERT()

splicebert.add_train_data({
	"sequence": sequence,
	"label": label,
}, sequence_len=256, batch_size=16, train_percentage=0.9)

splicebert.train(save_at_end="models/splicebert", lr=5e-5, epochs=3)
